function [Z] = subspace(data,d,para)
%SUBSPACE 此处显示有关此函数的摘要
%   此处显示详细说明
lambda=para.lambda;
alpha=para.alpha;
C = para.C;
...
%     fprintf('Running iter1 number - %d/%d \n',iter,maxIter);
    ...
    for uu=1:num_views
        ...
        Q{uu}=(theta(uu)*data{uu}*Z')*pinv(theta(uu)*Z*Z' + n*lambda*K + 1e-8*speye(d));
    ...
    for v=1:num_views
        QTQ=QTQ+Q{v}'*Q{v};
        QTX=QTX+Q{v}'*data{v};
        preloss(v,1)=norm(data{v}-Q{v}*Z,'fro')^2+num_views*C*norm(Z,'fro')^2;
